...
def main():
    ...
    def ImportProteinFiles(inputnames): ## This class imports the files into our workflow and makes a list of the replicates.
        ...
        for name in a:
            ...
            try:
                total = [np.genfromtxt(x, delimiter = ',', skip_header = 1, usecols = [0,2,5,3],  dtype = None) for x in glob.glob(str(argv[6]) + "/" + str(name) + "*.csv")] # Imports Protein Name, Spectral Count, Mol. Weight
            except ValueError:
                print("######################################")
